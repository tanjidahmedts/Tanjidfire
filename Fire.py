    ...
    try:
        ...
        if chos == '1' or chos == '01':
            ...
            if cot < 500000:
                ...
                ab = repr(bo)
                s = open(out, 'w')
                s.write('#Code Ecrypted By Tanjid Ahmed  \nimport marshal\nexec(marshal.loads(' + str(ab) + '))')
                s.close()
                while True:
                    if cot >= cout:
                        nz = open(out).read()
                        ...
                        nl = repr(bx)
                        ns = open(out, 'w')
                        ns.write('#Code Encrypted By Tanjid Ahmed\nimport marshal\nexec(marshal.loads(' + str(nl) + '))')
                        ns.close()
                        cout += 1
                        continue
                    break
            ...
            if cot < 500000:
                ...
                ab = repr(bo)
                s = open(out, 'w')
                s.write('#Code Encrypted By Tanjid Ahmed\nimport marshal\nexec(marshal.loads(' + str(ab) + '))')
                s.close()
                while True:
                    if cot >= cout:
                        nz = open(out).read()
                ...
                xn = open(out, 'w')
                xn.write("#Code Encrypted By Tanjid Ahmed\nimport base64\nexec(base64.b32decode('%s'))\n" % nl)
                xn.close()
                print '\x1b[34m[\x1b[31m!\x1b[34m] \x1b[37mDone \x1b[32m[ \x1b[37m%s \x1b[32m] \x1b[37m!' % out
                raw_input('%s[%s\xe2\x9d\x97%s] %sBack %s\xe2\x9e\xa4 %s' % (b, m, b, gr, i, cg))
                main()
            ...
            if cot < 40000:
                ...
                nl = base64.b32encode(mx)
                xn = open(out, 'w')
                xn.write("#Coded By Tanjid Ahmed\nimport base64\nexec(base64.b32decode('%s'))\n" % nl)
                xn.close()
                print '\x1b[34m[\x1b[31m!\x1b[34m] \x1b[37mDone \x1b[32m[ \x1b[37m%s \x1b[32m] \x1b[37m!' % out
                raw_input('%s[%s\xe2\x9d\x97%s] %sBack %s\xe2\x9e\xa4 %s' % (b, m, b, gr, i, cg))
                main()
